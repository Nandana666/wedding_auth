...
class _UsersTabState extends State<UsersTab> {
          ...
          child: FutureBuilder<List<QueryDocumentSnapshot>>(
            ...
            builder: (context, snapshot) {
              ...
              final filteredDocs = snapshot.data!.where((doc) {
                ...
                final role =
                    (data['role'] ??
                            (!data.containsKey('requestToAdmin')
                                ? 'vendor'
                                : 'user'))
                        .toString()
                        .toLowerCase();
                  ...
                  if (isMobile) {
                    return ListView.builder(
                      ...
                      itemBuilder: (context, index) =>
                          _userCard(filteredDocs[index], isMobile: true),
                    );
                  } else {
  ...
  Widget _userCard(QueryDocumentSnapshot doc, {bool isMobile = false}) {
    ...
    // FIX: Removed unused 'email' variable
    // final email = data['email'] ?? '';
    final role =
        data['role'] ??
        (!data.containsKey('requestToAdmin') ? 'vendor' : 'user');
    final avatarLetter = name.isNotEmpty ? name[0].toUpperCase() : '?';

    // ... rest of _userCard method is unchanged ...
    final cardContent = isMobile
        ? ListTile(
            ...
            trailing: Wrap(
              spacing: 8,
              children: [_viewDetailsButton(doc), _deleteButton(doc, role)],
            ),
          )
        : Row(
            ...
            children: [
              CircleAvatar(
                radius: 28,
                child: Text(avatarLetter, style: const TextStyle(fontSize: 20)),
              ),
      ...
      onPressed: () async {
        final confirm = await showDialog<bool>(
          ...
          builder: (_) => AlertDialog(
            ...
            content: const Text(
              'Are you sure you want to delete this user/vendor?',
            ),
            actions: [
              TextButton(
